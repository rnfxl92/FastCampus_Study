...
extension VoiceRecorderViewModel {
    ...
    func removeSelectedVoiceRecord() {
        guard let fileToRemove = selectedRecoredFile,
              let indexPathToRemove = recordedFiles.firstIndex(of: fileToRemove) else {
        ...
        do {
            ...
            recordedFiles.remove(at: indexPathToRemove)
...
extension VoiceRecorderViewModel {
    ...
    private func startRecording() {
        let fileURL = getDocumentDiretory().appendingPathComponent("새로운 녹음 \(recordedFiles.count + 1)")
        let settings = [
    ...
    private func getDocumentDiretory() -> URL {
...
extension VoiceRecorderViewModel {
    ...
    func getFileInfo(for url: URL) -> (Date?, TimeInterval?) {
        ...
        do {
            ...
            displayAlert(message: "선택된 음성 메모 파일의 재생 시간을 불러올 수 없습니다.")
        }
