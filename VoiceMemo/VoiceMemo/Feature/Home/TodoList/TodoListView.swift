...
struct TodoListView: View {
    ...
    @EnvironmentObject private var todoListModel: TodoListViewModel
    ...
    var body: some View {
        ZStack {
            // 투두 셀 리스트
            VStack {
                if !todoListModel.todos.isEmpty {
                    CustomNavigationBar(
                        isDiplayLeftBtn: false,
                        rightBtnAction: {
                            todoListModel.navigationRightBtnTapped()
                        },
                        rightBtnType: todoListModel.navigationBarRightBtnMode
                    )
                } else {
                    Spacer()
                        .frame(height: 30)
        ...
        .alert(
            "To do list \(todoListModel.removeTodosCount)개 삭제하시겠습니까?",
            isPresented: $todoListModel.isDisplayRemoveTodoAlert
        ) {
            Button("삭제", role: .destructive) {
                todoListModel.removeBtnTapped()
            }
            Button("취소", role: .cancel) { }
        }
        .onChange(
            of: todoListModel.todos,
            perform: { todos in
